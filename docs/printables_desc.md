# Modular French Cleat (MFC) System (Parametric, OpenGrid Compatible)

> **A fully parametric, modular 3D-printed French cleat tool organization system designed for 73 mm rails, featuring rigid trapezoidal locking grooves, OpenGrid/Gridfinity compatibility, and M3 hardware mounting.**

---

## Overview

This project provides the **Modular French Cleat (MFC) System**—a robust, standardized, and fully parametric French cleat tool organization ecosystem for workshops, maker spaces, and garages. 

Most French cleat models available online suffer from two key problems:
1. They integrate the cleat directly into the tool holder, forcing awkward print orientations that lead to weak layer lines across the load-bearing cleat bevel, or lots of supports.
...
* **28 mm Grid Standard:** Built upon the 28 mm pitch shared with **OpenGrid** (by David D) and relatively compatible with **Gridfinity** (by Zack Freedman).
...
> The pre-rendered STL and STEP files provided in this release are dimensioned specifically for **French cleat wall strips that are 73 mm (approx. 2-7/8") high**, with a standard 45° top bevel and nominal 19 mm (3/4") thickness. They also assume M3 hardware.
>
> If your workshop French cleat rails are **any height other than 73 mm**:
> 1. **Best & Fastest Option — Custom Generation via GitHub:**  
>    Visit the open-source repository at [https://github.com/atiretoo/french_cleats](https://github.com/atiretoo/french_cleats). The entire ecosystem is generated programmatically using Python and CadQuery. You can simply specify your rail height (e.g. `--rail-height 65.0` or `--rail-height 89.0`) and generate perfectly customized cleats and tool holders for your exact walls in seconds. Change the hardware to M4! 
> 2. **Alternative — Message Me:**  
>    You can send me a message here on Printables with your cleat dimensions. I will do my best to render and upload models for other rail sizes, hardware, or tool dimensions when time permits, but please be aware that responses may be slow.
...
* **Top Cleats (1U, 2U, 3U, 4U):** Sit on the wall rail's 45° top bevel. Incorporates the self-aligning trapezoidal ridge and rear slots for standard M3 hex nuts.
* **Bottom Cleats (1U, 2U, 3U, 4U):** Sit flush against the bottom edge of the rail to keep tools perfectly plumb and eliminate peel-away torque.
...
* **Gridfinity Shelves (3x3U, 6x3U, 6x6U):** Mount Zack Freedman's Gridfinity storage bins directly to your French cleat walls.
* **OpenGrid Adapters (1U, 2U, 3U, 4U):** Converts any of these tool holders for use on OpenGrid compatible systems, where a 28mm grid is used.
...
* **Material:** **PETG**, **ABS**, or **ASA** is strongly recommended for workshop longevity, creep resistance under load, and impact toughness. **PLA** is suitable for lighter hand tool organizers. My current testing is mostly Elegoo **PETG** and **PLA** printed on a Prusa Core One.
My test results used the following settings; these are not necessarily optimal. Further testing is underway.
* **Perimeters / Walls:** 2 walls (approx. 1.6–2.0 mm wall thickness).
...
## Attributions & Sources of Inspiration

This project stands on the shoulders of giants. While designing this system, I built upon the brilliant work already done by many, synthesizing the best aspects of several existing systems into a unified standard:

* **Frenchfinity** by Bastelsaal ([frenchfinity.xyz](https://frenchfinity.xyz/)): Inspired the excellent concept of separating the cleat base from the tool holder itself. Separating the components is what enables printing on edge for maximum layer-line shear strength across the 45° cleat bevel.
* **Screw & Ridge Locking Inspiration** by shoooo ([@shoooo_1013338 on Printables](https://www.printables.com/@shoooo_1013338)): Heavily inspired by shoooo's clever use of registration ridges and screws. I standardized this concept into self-aligning trapezoidal grooves for rock-solid, repeatable locking.
* **OpenGrid** by David D ([Printables Model #1214361](https://www.printables.com/model/1214361-opengrid-walldesk-mounting-framework-and-ecosystem)): Provided the 28 mm modular pitch standard adopted across all backplates, cleats, and adapters.
* **Gridfinity** by Zack Freedman ([YouTube](https://www.youtube.com/watch?v=ra_9zU-mnl8)): The gold standard for desktop and drawer organization, directly integrated into the French cleat shelf mounts.
...
## AI Disclosure: The "Centaur" Approach
